...
## Простой ответ
- Образ — слепок окружения; контейнер — его запущенный экземпляр в изоляции.
- Один и тот же образ стартует одинаково на любой машине с Docker.
...
1. `docker run -p 8080:80 nginx:alpine`.
2. `docker compose up -d` для старта приложения и БД.

## Доп. теория

1. Контейнеры используют Linux namespaces и cgroups для изоляции.
2. Образы состоят из слоёв; переиспользование слоёв ускоряет сборку.
