...
## Примеры
```text
Strategy: разные способы оплаты.
Adapter: подключить чужой API через свой интерфейс.
```

## Доп. теория
- Паттерны — словарь решений, а не обязательный «чек‑лист».
- Важно не переусложнять: иногда проще обычная функция.
