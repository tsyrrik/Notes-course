...
Простыми словами: браузер → `public/index.php` → приложение/контейнер → HTTP-ядро → bootstrappers → middleware → маршрутизатор → контроллер/замыкание → ответ → обратный проход через middleware → отправка в браузер.
